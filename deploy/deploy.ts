...
async function main() {
  ...
  // Deploy the plain PrivacyAuction contract
  const PrivacyAuctionFactory = await hardhatEthers.getContractFactory("PrivacyAuction", wallet);
  const factory = await PrivacyAuctionFactory.deploy();
  await factory.waitForDeployment();

  const deployedAddress = (factory as any).target || (factory as any).address;
  console.log("PrivacyAuction contract deployed at:", deployedAddress);
  ...
  if (!fs.existsSync(frontendConfigDir)) {
    ...
    try {
      ...
      fs.copyFileSync(artifactPath, path.join(targetAbiPath, "PrivacyAuction.json"));
      console.log("Copied ABI to frontend/web/src/abi/PrivacyAuction.json");
    } catch (e) {
      console.warn(
        "Failed to copy ABI automatically. Please copy artifacts/.../PrivacyAuction.json manually to frontend/web/src/abi/PrivacyAuction.json",
        e
      );
    }
  }
